...
class LineaService {
    async obtenerLineasService(params = {}) {
        const { q = "", page = 1, limit = 15, estado } = params;
        const where = [];
        const values = [];
        let idx = 1;
        if (q) {
            where.push(`nombre ILIKE $${idx++}`);
            values.push(`%${q}%`);
        }
        if (estado) {
    ...
    async obtenerLineasConRutasService(params = {}) {
        const { q = "", page = 1, limit = 15, estado } = params;
        const where = [];
        const values = [];
        let idx = 1;
        if (q) {
            where.push(`l.nombre ILIKE $${idx++}`);
            values.push(`%${q}%`);
        }
        if (estado) {
            where.push(`l.estado = $${idx++}`);
            values.push(estado);
        }
        values.push(limit, (page - 1) * limit);
        const sql = `
